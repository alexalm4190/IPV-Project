...
imseq1 = struct('rgb', {}, 'depth', {});
imseq2 = struct('rgb', {}, 'depth', {});
for i=1:length(d1),
    imseq1(i).rgb=[base_data_dir r1(i).name];
    imseq2(i).rgb=[base_data_dir r2(i).name];
    imseq1(i).depth=[base_data_dir d1(i).name];
    imseq2(i).depth=[base_data_dir d2(i).name];
end
%load calibration data
load cameraparametersAsus;
%WORKING DIRECTORY - location of all program directories  
basedir=pwd;
projs=dir(basedir);
% LOGFILE
outputdir='lab1/';
    ...
    for i=3:length(projs),
        ...
        if projs(i).isdir&& ~strcmp(projs(i).name,'.')&& ~strcmp(projs(i).name , '..'),
            ...
            if exist('track3D_part2.m'),
                corre='[objects, cam2toW] = track3D_part2( imseq1, imseq2, cam_params);';
                try,
...
for i=1:size(texto,2),
    ...
    if strcmp(texto{2,i}(1:2),'OK'),
       obj=caminho{1,i};
       objsinframe=zeros(length(obj),length(imseq1));
       for j=1:length(obj),
